...
export class PurchaseOrderController {
  ...
  findAll(
    ...
    if (user.role === Role.Vendor) {
      return this.poService.findAll(user.vendorId ?? '__none__');
    }
  ...
  @Post(':id/acknowledge')
  async acknowledge(@Param('id') id: string, @CurrentUser() user: AuthUser) {
    const po = await this.poService.findOne(id);
    if (!po) throw new NotFoundException();
    this.assertOwnership(user, po);
  ...
  private assertOwnership(user: AuthUser, po: any) {
    ...
    // po.vendor is populated; fall back to raw id if not.
    const ownerId = String(po.vendor?._id ?? po.vendor);
    if (!po.vendor || ownerId !== user.vendorId) {
